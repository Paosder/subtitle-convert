  ...
  _createClass(SubtitleConverter, [{
    ...
    value: function apply(cueList) {
      if ((typeof cueList === 'undefined' ? 'undefined' : _typeof(cueList)) === 'object') {
        var arr = cueList;
        this.parsed.cueList.forEach(function (el, i) {
          arr[i].endTime = el.end.getUTCMinutes() * 60 + el.end.getUTCSeconds() + el.end.getUTCMilliseconds() * 0.01;
          arr[i].startTime = el.start.getUTCMinutes() * 60 + el.start.getUTCSeconds() + el.start.getUTCMilliseconds() * 0.01;
          arr[i].text = el.text;
        });
        return true;
      }
